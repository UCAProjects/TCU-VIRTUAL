<?php
  ...
  $id = $_POST["id"];
  $query =  "SELECT R.Observaciones, E.descripcion as estado, R.version, DATE_FORMAT(R.fecha_revision,'%d-%m-%y / %h:%i:%s') AS fecha FROM tigrupou_tcu.revision_resumen_ejecutivo as R JOIN tigrupou_tcu.estado as E ON R.estado LIKE E.codigo WHERE resumen_ejecutivo LIKE $id AND version like ( SELECT max(version) FROM tigrupou_tcu.revision_resumen_ejecutivo WHERE resumen_ejecutivo LIKE $id and rol like 2) and rol like 2";
