    ...
    foreach($_FILES['uploadedSolicitud']['tmp_name'] as $key => $tmp_name)
    {
      ...
      if($_FILES["uploadedSolicitud"]["name"][$key]) {
        ...
        if(move_uploaded_file($source, $target_pathSolicitud)) {
          //$querycartaRealizacion= "insert into tigrupou_agenda_universitaria.anexos(url,actividad_id,tipoDocumento,restringido) values(\"$target_path\",$id,\"$tipo\",$restringido);";
   ...
   foreach($_FILES["uploadedAceptacion"]['tmp_name'] as $key => $tmp_name)
   {
     ...
     if($_FILES["uploadedAceptacion"]["name"][$key]) {
       ...
       if(move_uploaded_file($source, $target_pathAceptacion)) {
         //$queryAceptacion = "insert into tigrupou_agenda_universitaria.anexos(url,actividad_id,tipoDocumento,restringido) values(\"$target_path\",$id,\"$tipo\",$restringido);";
         $boolAceptacion = true;
       }
       else {
         echo "Ha ocurrido un error, por favor inténtelo de nuevo.<br>";
